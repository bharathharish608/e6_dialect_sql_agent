...
```sql
DATE_TRUNC( <expression1>, <expression2> )
```
...
- **Type**: STRING
- **Required**: Yes
- **Description**: Date part to truncate to
### expression2
- **Type**: TIMESTAMP
- **Required**: Yes
...
- **Description**: Returns the result of the DATE_TRUNC operation.
- **NULL Handling**: Function behavior with NULL inputs depends on the specific operation.

## Usage Notes

- Truncates a timestamp to a specified precision
- Function accepts STRING, TIMESTAMP
...
Query:
```sql
SELECT DATE_TRUNC('MONTH', '2023-01-15 10:30:00') -- Returns '2023-01-01 00:00:00'
```
...
This example demonstrates basic usage of the DATE_TRUNC function.
...
Query:
```sql
SELECT product_id, product_name, DATE_TRUNC(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how DATE_TRUNC handles NULL values.
...
Query:
```sql
-- Practical example using DATE_TRUNC
...
This example shows a practical application of the DATE_TRUNC function.