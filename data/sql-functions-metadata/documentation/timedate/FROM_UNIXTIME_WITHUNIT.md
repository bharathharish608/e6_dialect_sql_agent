...
Converts Unix timestamp to a timestamp with specified unit

## Syntax

```sql
FROM_UNIXTIME_WITHUNIT( <expression1>, <expression2> )
```
...
- **Type**: STRING
- **Required**: Yes
- **Description**: time unit (seconds/milliseconds/microseconds)

## Returns

- **Type**: TIMESTAMP NULLABLE
- **Description**: Returns the result of the FROM_UNIXTIME_WITHUNIT operation.
- **NULL Handling**: Function behavior with NULL inputs depends on the specific operation.

## Usage Notes

- Converts Unix timestamp to a timestamp with specified unit
- Function accepts NUMERIC, STRING
- Return type is TIMESTAMP NULLABLE
...
Query:
```sql
SELECT FROM_UNIXTIME_WITHUNIT(1672531200000, 'milliseconds') -- From milliseconds
```
...
This example demonstrates basic usage of the FROM_UNIXTIME_WITHUNIT function.
...
Query:
```sql
SELECT product_id, product_name, FROM_UNIXTIME_WITHUNIT(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how FROM_UNIXTIME_WITHUNIT handles NULL values.
...
Query:
```sql
-- Practical example using FROM_UNIXTIME_WITHUNIT
...
This example shows a practical application of the FROM_UNIXTIME_WITHUNIT function.