...
Adds an interval to a date or timestamp (SQL Server compatible)

## Syntax

```sql
DATEADD( <expression1>, <expression2>, <expression3> )
```
...
- **Description**: Returns the result of the DATEADD operation.
...
Query:
```sql
SELECT DATEADD('month', 3, '2023-01-01') -- Adds 3 months
```
...
This example demonstrates basic usage of the DATEADD function.
...
Query:
```sql
SELECT product_id, product_name, DATEADD(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how DATEADD handles NULL values.
...
Query:
```sql
-- Practical example using DATEADD
...
This example shows a practical application of the DATEADD function.