...
```sql
DATEDIFF( <expression1>, <expression2> [ , <expression3> ] )
```
...
- **Description**: Returns the result of the DATEDIFF operation.
- **NULL Handling**: Function behavior with NULL inputs depends on the specific operation.

## Usage Notes

- Returns the difference between two dates
- Function accepts DATE/TIMESTAMP, DATE/TIMESTAMP or STRING, DATE/TIMESTAMP, DATE/TIMESTAMP
...
Query:
```sql
SELECT DATEDIFF('2023-01-15', '2023-01-01') -- Returns 14 (days)
```
...
This example demonstrates basic usage of the DATEDIFF function.
...
Query:
```sql
SELECT product_id, product_name, DATEDIFF(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how DATEDIFF handles NULL values.
...
Query:
```sql
-- Practical example using DATEDIFF
...
This example shows a practical application of the DATEDIFF function.