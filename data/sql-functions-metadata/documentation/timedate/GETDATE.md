...
Returns the current timestamp (SQL Server compatible)
...
- **Type**: TIMESTAMP
- **Description**: Returns the result of the GETDATE operation.
...
This example demonstrates basic usage of the GETDATE function.
...
Query:
```sql
SELECT product_id, product_name, GETDATE(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how GETDATE handles NULL values.
...
Query:
```sql
-- Practical example using GETDATE
...
This example shows a practical application of the GETDATE function.