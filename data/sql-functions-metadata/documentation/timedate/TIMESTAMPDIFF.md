...
```sql
TIMESTAMPDIFF( <expression1>, <expression2>, <expression3> )
```
...
- **Description**: Returns the result of the TIMESTAMPDIFF operation.
- **NULL Handling**: Function behavior with NULL inputs depends on the specific operation.

## Usage Notes

- Returns the difference between two timestamps
- Function accepts STRING, TIMESTAMP, TIMESTAMP
...
Query:
```sql
SELECT TIMESTAMPDIFF('HOUR', '2023-01-01 10:00:00', '2023-01-01 15:00:00') -- Returns 5
```
...
This example demonstrates basic usage of the TIMESTAMPDIFF function.
...
Query:
```sql
SELECT product_id, product_name, TIMESTAMPDIFF(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how TIMESTAMPDIFF handles NULL values.
...
Query:
```sql
-- Practical example using TIMESTAMPDIFF
...
This example shows a practical application of the TIMESTAMPDIFF function.