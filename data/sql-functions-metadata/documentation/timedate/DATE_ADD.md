...
```sql
DATE_ADD( <expression1>, <expression2> [ , <expression3> ] )
```
...
- **Type**: TIMESTAMP or DATE NULLABLE
- **Description**: Returns the result of the DATE_ADD operation.
- **NULL Handling**: Function behavior with NULL inputs depends on the specific operation.

## Usage Notes

- Adds an interval to a date or timestamp
- Function accepts TIMESTAMP/DATE, INTEGER or STRING, INTEGER, TIMESTAMP
- Return type is TIMESTAMP or DATE NULLABLE
...
Query:
```sql
SELECT DATE_ADD(CURRENT_DATE, 7) -- Adds 7 days
```
...
This example demonstrates basic usage of the DATE_ADD function.
...
Query:
```sql
SELECT product_id, product_name, DATE_ADD(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how DATE_ADD handles NULL values.
...
Query:
```sql
-- Practical example using DATE_ADD
...
This example shows a practical application of the DATE_ADD function.