...
```sql
TIMESTAMP_ADD( <expression1>, <expression2>, <expression3> )
```
...
- **Description**: Returns the result of the TIMESTAMP_ADD operation.
- **NULL Handling**: Function behavior with NULL inputs depends on the specific operation.

## Usage Notes

- Adds an interval to a timestamp
- Function accepts STRING, INTEGER, TIMESTAMP
...
Query:
```sql
SELECT TIMESTAMP_ADD('HOUR', 2, '2023-01-01 10:00:00') -- Returns '2023-01-01 12:00:00'
```
...
This example demonstrates basic usage of the TIMESTAMP_ADD function.
...
Query:
```sql
SELECT product_id, product_name, TIMESTAMP_ADD(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how TIMESTAMP_ADD handles NULL values.
...
Query:
```sql
-- Practical example using TIMESTAMP_ADD
...
This example shows a practical application of the TIMESTAMP_ADD function.