...
```sql
DATE_SUB( <expression1>, <expression2> )
```
...
- **Type**: DATE NULLABLE
- **Description**: Returns the result of the DATE_SUB operation.
- **NULL Handling**: Function behavior with NULL inputs depends on the specific operation.

## Usage Notes

- Subtracts days from a date
- Function accepts DATE, INTEGER
...
Query:
```sql
SELECT DATE_SUB('2023-01-15', 7) -- Returns '2023-01-08'
```
...
This example demonstrates basic usage of the DATE_SUB function.
...
Query:
```sql
SELECT product_id, product_name, DATE_SUB(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how DATE_SUB handles NULL values.
...
Query:
```sql
-- Practical example using DATE_SUB
...
This example shows a practical application of the DATE_SUB function.