...
```sql
DATE_DIFF( <expression1>, <expression2>, <expression3> )
```
...
- **Description**: Returns the result of the DATE_DIFF operation.
- **NULL Handling**: Function behavior with NULL inputs depends on the specific operation.

## Usage Notes

- Returns the difference between two dates
- Function accepts STRING, DATE/TIMESTAMP, DATE/TIMESTAMP
...
Query:
```sql
SELECT DATE_DIFF('DAY', '2023-01-01', '2023-01-15') -- Returns 14
```
...
This example demonstrates basic usage of the DATE_DIFF function.
...
Query:
```sql
SELECT product_id, product_name, DATE_DIFF(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how DATE_DIFF handles NULL values.
...
Query:
```sql
-- Practical example using DATE_DIFF
...
This example shows a practical application of the DATE_DIFF function.