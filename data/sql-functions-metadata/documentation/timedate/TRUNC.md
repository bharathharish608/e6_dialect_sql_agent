...
- **Description**: Returns the result of the TRUNC operation.
- **NULL Handling**: Function behavior with NULL inputs depends on the specific operation.

## Usage Notes

- Truncates a timestamp to a specified precision
- Function accepts TIMESTAMP, STRING
...
Query:
```sql
SELECT TRUNC('2023-01-15 10:30:00', 'DAY') -- Returns '2023-01-15 00:00:00'
```
...
This example demonstrates basic usage of the TRUNC function.
...
Query:
```sql
SELECT product_id, product_name, TRUNC(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how TRUNC handles NULL values.
...
This example shows a practical application of the TRUNC function.