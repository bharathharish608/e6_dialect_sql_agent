...
- **Description**: Returns the result of the SLICE operation.
- **NULL Handling**: Function behavior with NULL inputs depends on the specific operation.

## Usage Notes

- Extracts a slice of an array
- Function accepts ARRAY, INTEGER, INTEGER
...
Query:
```sql
SELECT SLICE([1,2,3,4,5], 2, 3) -- Returns [2,3,4]
```
...
This example demonstrates basic usage of the SLICE function.
...
Query:
```sql
SELECT product_id, product_name, SLICE(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how SLICE handles NULL values.
...
This example shows a practical application of the SLICE function.