...
Returns the negative value (unary minus)
...
This example demonstrates basic usage of the NEGATIVE function.
...
Query:
```sql
SELECT product_id, product_name, NEGATIVE(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how NEGATIVE handles NULL values.
...
Query:
```sql
-- Practical example using NEGATIVE
...
This example shows a practical application of the NEGATIVE function.