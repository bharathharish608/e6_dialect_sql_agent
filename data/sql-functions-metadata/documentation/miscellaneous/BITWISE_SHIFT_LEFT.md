...
```sql
BITWISE_SHIFT_LEFT( <expression1>, <expression2>, <expression3> )
```
...
- **Description**: Returns the result of the BITWISE_SHIFT_LEFT operation.
...
Query:
```sql
SELECT BITWISE_SHIFT_LEFT(4, 2, 32) -- Returns 16 (100 << 2 = 10000)
```
...
This example demonstrates basic usage of the BITWISE_SHIFT_LEFT function.
...
Query:
```sql
SELECT product_id, product_name, BITWISE_SHIFT_LEFT(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how BITWISE_SHIFT_LEFT handles NULL values.
...
Query:
```sql
-- Practical example using BITWISE_SHIFT_LEFT
...
This example shows a practical application of the BITWISE_SHIFT_LEFT function.