...
Converts a timestamp to Unix timestamp

## Syntax

```sql
TO_UNIX_TIMESTAMP( <expression> )
```
...
- **Description**: Returns the result of the TO_UNIX_TIMESTAMP operation.
...
Query:
```sql
SELECT TO_UNIX_TIMESTAMP('2023-01-01 00:00:00') -- Returns 1672531200
```
...
This example demonstrates basic usage of the TO_UNIX_TIMESTAMP function.
...
Query:
```sql
SELECT product_id, product_name, TO_UNIX_TIMESTAMP(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how TO_UNIX_TIMESTAMP handles NULL values.
...
Query:
```sql
-- Practical example using TO_UNIX_TIMESTAMP
...
This example shows a practical application of the TO_UNIX_TIMESTAMP function.