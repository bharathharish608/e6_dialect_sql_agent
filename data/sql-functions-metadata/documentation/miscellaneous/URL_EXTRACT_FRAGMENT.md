...
Extracts the fragment (hash) from a URL

## Syntax

```sql
URL_EXTRACT_FRAGMENT( <expression> )
```
...
- **Description**: Returns the result of the URL_EXTRACT_FRAGMENT operation.
...
- Return type is Same as input, nullable
...
Query:
```sql
SELECT URL_EXTRACT_FRAGMENT('https://example.com/page#section') -- Returns 'section'
```
...
This example demonstrates basic usage of the URL_EXTRACT_FRAGMENT function.
...
Query:
```sql
SELECT product_id, product_name, URL_EXTRACT_FRAGMENT(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how URL_EXTRACT_FRAGMENT handles NULL values.
...
Query:
```sql
-- Practical example using URL_EXTRACT_FRAGMENT
...
This example shows a practical application of the URL_EXTRACT_FRAGMENT function.