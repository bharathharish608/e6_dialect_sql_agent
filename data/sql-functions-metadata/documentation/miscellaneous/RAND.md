...
- **Description**: Returns the result of the RAND operation.
...
Query:
```sql
SELECT RAND() -- Returns random value
```
...
This example demonstrates basic usage of the RAND function.
...
Query:
```sql
SELECT product_id, product_name, RAND(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how RAND handles NULL values.
...
This example shows a practical application of the RAND function.