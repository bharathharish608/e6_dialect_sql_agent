...
- **Description**: Returns the result of the URL_EXTRACT_HOST operation.
...
Query:
```sql
SELECT URL_EXTRACT_HOST('https://www.example.com/path') -- Returns 'www.example.com'
```
...
This example demonstrates basic usage of the URL_EXTRACT_HOST function.
...
Query:
```sql
SELECT product_id, product_name, URL_EXTRACT_HOST(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how URL_EXTRACT_HOST handles NULL values.
...
Query:
```sql
-- Practical example using URL_EXTRACT_HOST
...
This example shows a practical application of the URL_EXTRACT_HOST function.