...
Returns NULL if two values are equal, else returns first value
...
- **Type**: Same as first argument, force nullable
- **Description**: Returns the result of the NULLIF operation.
- **NULL Handling**: Function behavior with NULL inputs depends on the specific operation.

## Usage Notes

- Returns NULL if two values are equal, else returns first value
- Function accepts COMPARABLE, COMPARABLE
- Return type is Same as first argument, force nullable
...
This example demonstrates basic usage of the NULLIF function.
...
Query:
```sql
SELECT product_id, product_name, NULLIF(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how NULLIF handles NULL values.
...
Query:
```sql
-- Practical example using NULLIF
...
This example shows a practical application of the NULLIF function.