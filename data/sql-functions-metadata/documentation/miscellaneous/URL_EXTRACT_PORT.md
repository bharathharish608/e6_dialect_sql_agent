...
- **Description**: Returns the result of the URL_EXTRACT_PORT operation.
...
Query:
```sql
SELECT URL_EXTRACT_PORT('https://example.com:8080/path') -- Returns 8080
```
...
This example demonstrates basic usage of the URL_EXTRACT_PORT function.
...
Query:
```sql
SELECT product_id, product_name, URL_EXTRACT_PORT(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how URL_EXTRACT_PORT handles NULL values.
...
Query:
```sql
-- Practical example using URL_EXTRACT_PORT
...
This example shows a practical application of the URL_EXTRACT_PORT function.