...
```sql
TO_BASE( <expression1>, <expression2> )
```
...
- **Description**: Returns the result of the TO_BASE operation.
...
This example demonstrates basic usage of the TO_BASE function.
...
Query:
```sql
SELECT product_id, product_name, TO_BASE(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how TO_BASE handles NULL values.
...
Query:
```sql
-- Practical example using TO_BASE
...
This example shows a practical application of the TO_BASE function.