...
```sql
URL_EXTRACT_PROTOCOL( <expression> )
```
...
- **Description**: Returns the result of the URL_EXTRACT_PROTOCOL operation.
...
Query:
```sql
SELECT URL_EXTRACT_PROTOCOL('https://example.com') -- Returns 'https'
```
...
This example demonstrates basic usage of the URL_EXTRACT_PROTOCOL function.
...
Query:
```sql
SELECT product_id, product_name, URL_EXTRACT_PROTOCOL(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how URL_EXTRACT_PROTOCOL handles NULL values.
...
Query:
```sql
-- Practical example using URL_EXTRACT_PROTOCOL
...
This example shows a practical application of the URL_EXTRACT_PROTOCOL function.