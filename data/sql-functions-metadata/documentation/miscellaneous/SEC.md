...
Returns the secant of an angle (1/cos(x))
...
- **Description**: Returns the result of the SEC operation.
...
This example demonstrates basic usage of the SEC function.
...
Query:
```sql
SELECT product_id, product_name, SEC(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how SEC handles NULL values.
...
This example shows a practical application of the SEC function.