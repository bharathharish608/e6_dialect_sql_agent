...
```sql
BITWISE_OR( <expression1>, <expression2> )
```
...
- **Description**: Returns the result of the BITWISE_OR operation.
...
Query:
```sql
SELECT BITWISE_OR(12, 10) -- Returns 14 (1100 | 1010 = 1110)
```
...
This example demonstrates basic usage of the BITWISE_OR function.
...
Query:
```sql
SELECT product_id, product_name, BITWISE_OR(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how BITWISE_OR handles NULL values.
...
Query:
```sql
-- Practical example using BITWISE_OR
...
This example shows a practical application of the BITWISE_OR function.