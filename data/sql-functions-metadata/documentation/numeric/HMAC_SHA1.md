...
Returns HMAC-SHA1 authentication code

## Syntax

```sql
HMAC_SHA1( <expression1>, <expression2> [ , <expressionN> ... ] )
```
...
- **Description**: Returns the result of the HMAC_SHA1 operation.
...
Query:
```sql
SELECT HMAC_SHA1('message', 'secret') -- Returns HMAC
```
...
This example demonstrates basic usage of the HMAC_SHA1 function.
...
Query:
```sql
SELECT product_id, product_name, HMAC_SHA1(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how HMAC_SHA1 handles NULL values.
...
Query:
```sql
-- Practical example using HMAC_SHA1
...
This example shows a practical application of the HMAC_SHA1 function.