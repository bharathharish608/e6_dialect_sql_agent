...
```sql
SHIFTRIGHT( <expression1>, <expression2> )
```
...
- **Description**: Returns the result of the SHIFTRIGHT operation.
...
Query:
```sql
SELECT SHIFTRIGHT(32, 2) -- Returns 8
```
...
This example demonstrates basic usage of the SHIFTRIGHT function.
...
Query:
```sql
SELECT product_id, product_name, SHIFTRIGHT(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how SHIFTRIGHT handles NULL values.
...
Query:
```sql
-- Practical example using SHIFTRIGHT
...
This example shows a practical application of the SHIFTRIGHT function.