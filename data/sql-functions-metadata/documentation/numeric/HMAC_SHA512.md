...
Returns HMAC-SHA512 authentication code

## Syntax

```sql
HMAC_SHA512( <expression1>, <expression2> [ , <expressionN> ... ] )
```
...
- **Description**: Returns the result of the HMAC_SHA512 operation.
...
Query:
```sql
SELECT HMAC_SHA512('message', 'secret') -- Returns HMAC
```
...
This example demonstrates basic usage of the HMAC_SHA512 function.
...
Query:
```sql
SELECT product_id, product_name, HMAC_SHA512(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how HMAC_SHA512 handles NULL values.
...
Query:
```sql
-- Practical example using HMAC_SHA512
...
This example shows a practical application of the HMAC_SHA512 function.