...
Parses Presto-style data size strings to bytes

## Syntax

```sql
PARSE_PRESTO_DATA_SIZE( <expression> )
```
...
- **Type**: STRING
- **Required**: Yes
- **Description**: Data size string (e.g., '1GB', '500MB')

## Returns

- **Type**: BIGINT NULLABLE
- **Description**: Returns the result of the PARSE_PRESTO_DATA_SIZE operation.
...
Query:
```sql
SELECT PARSE_PRESTO_DATA_SIZE('1GB') -- Returns 1073741824
```
...
This example demonstrates basic usage of the PARSE_PRESTO_DATA_SIZE function.
...
Query:
```sql
SELECT product_id, product_name, PARSE_PRESTO_DATA_SIZE(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how PARSE_PRESTO_DATA_SIZE handles NULL values.
...
Query:
```sql
-- Practical example using PARSE_PRESTO_DATA_SIZE
...
This example shows a practical application of the PARSE_PRESTO_DATA_SIZE function.