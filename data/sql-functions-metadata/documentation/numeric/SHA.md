...
- **Description**: Returns the result of the SHA operation.
...
Query:
```sql
SELECT SHA('hello') -- Returns SHA-1 hash
```
...
This example demonstrates basic usage of the SHA function.
...
Query:
```sql
SELECT product_id, product_name, SHA(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how SHA handles NULL values.
...
This example shows a practical application of the SHA function.