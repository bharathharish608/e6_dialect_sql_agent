...
Returns the bit at specified position

## Syntax

```sql
GETBIT( <expression1>, <expression2> )
```
...
- **Description**: Returns the result of the GETBIT operation.
...
Query:
```sql
SELECT GETBIT(5, 0) -- Returns 1 (bit 0 of 101)
```
...
This example demonstrates basic usage of the GETBIT function.
...
Query:
```sql
SELECT product_id, product_name, GETBIT(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how GETBIT handles NULL values.
...
Query:
```sql
-- Practical example using GETBIT
...
This example shows a practical application of the GETBIT function.