...
Returns HMAC-MD5 authentication code

## Syntax

```sql
HMAC_MD5( <expression1>, <expression2> )
```
...
- **Description**: Returns the result of the HMAC_MD5 operation.
...
Query:
```sql
SELECT HMAC_MD5('message', 'secret') -- Returns HMAC
```
...
This example demonstrates basic usage of the HMAC_MD5 function.
...
Query:
```sql
SELECT product_id, product_name, HMAC_MD5(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how HMAC_MD5 handles NULL values.
...
Query:
```sql
-- Practical example using HMAC_MD5
...
This example shows a practical application of the HMAC_MD5 function.