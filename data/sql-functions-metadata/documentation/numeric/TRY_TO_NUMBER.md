...
Attempts to convert string to number, returns NULL on failure

## Syntax

```sql
TRY_TO_NUMBER( <expression1>, <expression2> )
```
...
- **Description**: Returns the result of the TRY_TO_NUMBER operation.
...
- Return type is DOUBLE NULLABLE
...
Query:
```sql
SELECT TRY_TO_NUMBER('abc', '999') -- Returns NULL
```
...
This example demonstrates basic usage of the TRY_TO_NUMBER function.
...
Query:
```sql
SELECT product_id, product_name, TRY_TO_NUMBER(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how TRY_TO_NUMBER handles NULL values.
...
Query:
```sql
-- Practical example using TRY_TO_NUMBER
...
This example shows a practical application of the TRY_TO_NUMBER function.