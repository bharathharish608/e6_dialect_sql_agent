...
```sql
BITWISE_AND( <expression1>, <expression2> )
```
...
- **Description**: Returns the result of the BITWISE_AND operation.
...
Query:
```sql
SELECT BITWISE_AND(12, 10) -- Returns 8 (1100 & 1010 = 1000)
```
...
This example demonstrates basic usage of the BITWISE_AND function.
...
Query:
```sql
SELECT product_id, product_name, BITWISE_AND(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how BITWISE_AND handles NULL values.
...
Query:
```sql
-- Practical example using BITWISE_AND
...
This example shows a practical application of the BITWISE_AND function.