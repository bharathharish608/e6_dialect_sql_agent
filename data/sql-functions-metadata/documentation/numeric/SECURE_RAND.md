...
- **Type**: DOUBLE
- **Description**: Returns the result of the SECURE_RAND operation.
...
Query:
```sql
SELECT SECURE_RAND() -- Returns secure random value
```
...
This example demonstrates basic usage of the SECURE_RAND function.
...
Query:
```sql
SELECT product_id, product_name, SECURE_RAND(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how SECURE_RAND handles NULL values.
...
Query:
```sql
-- Practical example using SECURE_RAND
...
This example shows a practical application of the SECURE_RAND function.