...
```sql
COSINE_DISTANCE( <expression1>, <expression2> )
```
...
- **Type**: FLOAT NULLABLE
- **Description**: Returns the result of the COSINE_DISTANCE operation.
...
Query:
```sql
SELECT COSINE_DISTANCE('[1,2,3]', '[4,5,6]') -- Returns cosine distance
```
...
This example demonstrates basic usage of the COSINE_DISTANCE function.
...
Query:
```sql
SELECT product_id, product_name, COSINE_DISTANCE(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how COSINE_DISTANCE handles NULL values.
...
Query:
```sql
-- Practical example using COSINE_DISTANCE
...
This example shows a practical application of the COSINE_DISTANCE function.