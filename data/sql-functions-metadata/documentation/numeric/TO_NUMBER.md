...
Converts a string to a number using a format

## Syntax

```sql
TO_NUMBER( <expression1>, <expression2> )
```
...
- **Description**: Returns the result of the TO_NUMBER operation.
...
Query:
```sql
SELECT TO_NUMBER('123.45', '999.99') -- Returns 123.45
```
...
This example demonstrates basic usage of the TO_NUMBER function.
...
Query:
```sql
SELECT product_id, product_name, TO_NUMBER(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how TO_NUMBER handles NULL values.
...
Query:
```sql
-- Practical example using TO_NUMBER
...
This example shows a practical application of the TO_NUMBER function.