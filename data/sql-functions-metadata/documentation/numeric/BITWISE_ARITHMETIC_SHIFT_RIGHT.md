...
```sql
BITWISE_ARITHMETIC_SHIFT_RIGHT( <expression1>, <expression2> )
```
...
- **Description**: Returns the result of the BITWISE_ARITHMETIC_SHIFT_RIGHT operation.
- **NULL Handling**: Function behavior with NULL inputs depends on the specific operation.

## Usage Notes

- Arithmetic right shift (preserves sign)
- Function accepts NUMERIC, NUMERIC
...
Query:
```sql
SELECT BITWISE_ARITHMETIC_SHIFT_RIGHT(-16, 2) -- Returns -4
```
...
This example demonstrates basic usage of the BITWISE_ARITHMETIC_SHIFT_RIGHT function.
...
Query:
```sql
SELECT product_id, product_name, BITWISE_ARITHMETIC_SHIFT_RIGHT(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how BITWISE_ARITHMETIC_SHIFT_RIGHT handles NULL values.
...
Query:
```sql
-- Practical example using BITWISE_ARITHMETIC_SHIFT_RIGHT
...
This example shows a practical application of the BITWISE_ARITHMETIC_SHIFT_RIGHT function.