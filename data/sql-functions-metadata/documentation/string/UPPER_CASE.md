...
- **Description**: Returns the result of the UPPER_CASE operation.
...
Query:
```sql
SELECT UPPER_CASE('hello') -- Returns 'HELLO'
```
...
This example demonstrates basic usage of the UPPER_CASE function.
...
Query:
```sql
SELECT product_id, product_name, UPPER_CASE(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how UPPER_CASE handles NULL values.
...
Query:
```sql
-- Practical example using UPPER_CASE
...
This example shows a practical application of the UPPER_CASE function.