...
- **Description**: Returns the result of the REPLACE operation.
- **NULL Handling**: Function behavior with NULL inputs depends on the specific operation.

## Usage Notes

- Replaces all occurrences of a substring within a string
- Function accepts STRING, STRING, STRING
...
This example demonstrates basic usage of the REPLACE function.
...
Query:
```sql
SELECT product_id, product_name, REPLACE(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how REPLACE handles NULL values.
...
This example shows a practical application of the REPLACE function.