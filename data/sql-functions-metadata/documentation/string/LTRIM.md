...
Removes leading spaces or specified characters from a string

## Syntax

```sql
LTRIM( <expression> [ , <parameter2> ] )
```
...
- **Description**: Returns the result of the LTRIM operation.
...
This example demonstrates basic usage of the LTRIM function.
...
Query:
```sql
SELECT product_id, product_name, LTRIM(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how LTRIM handles NULL values.
...
This example shows a practical application of the LTRIM function.