...
```sql
LPAD( <expression1>, <expression2> [ , <expression3> ] )
```
...
- **Description**: Returns the result of the LPAD operation.
...
This example demonstrates basic usage of the LPAD function.
...
Query:
```sql
SELECT product_id, product_name, LPAD(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how LPAD handles NULL values.
...
This example shows a practical application of the LPAD function.