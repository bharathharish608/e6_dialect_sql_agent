...
Removes trailing spaces or specified characters from a string

## Syntax

```sql
RTRIM( <expression> [ , <parameter2> ] )
```
...
- **Description**: Returns the result of the RTRIM operation.
...
This example demonstrates basic usage of the RTRIM function.
...
Query:
```sql
SELECT product_id, product_name, RTRIM(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how RTRIM handles NULL values.
...
This example shows a practical application of the RTRIM function.