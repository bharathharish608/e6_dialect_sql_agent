...
```sql
RPAD( <expression1>, <expression2> [ , <expression3> ] )
```
...
- **Description**: Returns the result of the RPAD operation.
- **NULL Handling**: Function behavior with NULL inputs depends on the specific operation.

## Usage Notes

- Pads a string on the right to a specified length
- Function accepts STRING, INTEGER, [STRING]
...
This example demonstrates basic usage of the RPAD function.
...
Query:
```sql
SELECT product_id, product_name, RPAD(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how RPAD handles NULL values.
...
This example shows a practical application of the RPAD function.