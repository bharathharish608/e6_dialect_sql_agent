...
- **Type**: STRING
- **Required**: Yes
- **Description**: Base32 string to decode

## Returns

- **Type**: VARCHAR varying, nullable
- **Description**: Returns the result of the FROM_BASE32 operation.
...
Query:
```sql
SELECT FROM_BASE32('NBSWY3DP') -- Returns 'hello'
```
...
This example demonstrates basic usage of the FROM_BASE32 function.
...
Query:
```sql
SELECT product_id, product_name, FROM_BASE32(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how FROM_BASE32 handles NULL values.
...
Query:
```sql
-- Practical example using FROM_BASE32
...
This example shows a practical application of the FROM_BASE32 function.