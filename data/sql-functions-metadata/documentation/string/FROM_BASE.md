...
Converts a string from the specified base to decimal

## Syntax

```sql
FROM_BASE( <expression1>, <expression2> )
```
...
- **Description**: Returns the result of the FROM_BASE operation.
- **NULL Handling**: Function behavior with NULL inputs depends on the specific operation.

## Usage Notes

- Converts a string from the specified base to decimal
- Function accepts STRING, INTEGER
...
This example demonstrates basic usage of the FROM_BASE function.
...
Query:
```sql
SELECT product_id, product_name, FROM_BASE(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how FROM_BASE handles NULL values.
...
Query:
```sql
-- Practical example using FROM_BASE
...
This example shows a practical application of the FROM_BASE function.