...
- **Description**: Returns the result of the FROM_BASE64 operation.
...
Query:
```sql
SELECT FROM_BASE64('aGVsbG8=') -- Returns 'hello'
```
...
This example demonstrates basic usage of the FROM_BASE64 function.
...
Query:
```sql
SELECT product_id, product_name, FROM_BASE64(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how FROM_BASE64 handles NULL values.
...
Query:
```sql
-- Practical example using FROM_BASE64
...
This example shows a practical application of the FROM_BASE64 function.