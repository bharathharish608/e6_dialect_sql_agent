...
- **Type**: STRING
- **Required**: Yes
- **Description**: URL-safe Base64 string to decode

## Returns

- **Type**: VARCHAR varying, nullable
- **Description**: Returns the result of the FROM_BASE64URL operation.
...
- Return type is VARCHAR varying, nullable
...
Query:
```sql
SELECT FROM_BASE64URL('aGVsbG8') -- Returns decoded string
```
...
This example demonstrates basic usage of the FROM_BASE64URL function.
...
Query:
```sql
SELECT product_id, product_name, FROM_BASE64URL(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how FROM_BASE64URL handles NULL values.
...
Query:
```sql
-- Practical example using FROM_BASE64URL
...
This example shows a practical application of the FROM_BASE64URL function.