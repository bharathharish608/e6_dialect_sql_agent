...
- **Description**: Returns the result of the IF operation.
- **NULL Handling**: Function behavior with NULL inputs depends on the specific operation.

## Usage Notes

- Returns one of two values based on a condition
- Function accepts BOOLEAN, ANY, ANY
...
This example demonstrates basic usage of the IF function.
...
Query:
```sql
SELECT product_id, product_name, IF(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how IF handles NULL values.
...
This example shows a practical application of the IF function.