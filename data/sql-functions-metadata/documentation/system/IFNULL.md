...
- **Type**: Least restrictive type, nullable
- **Description**: Returns the result of the IFNULL operation.
- **NULL Handling**: Function behavior with NULL inputs depends on the specific operation.

## Usage Notes

- Returns the second value if the first is null
- Function accepts SAME, SAME
- Return type is Least restrictive type, nullable
...
This example demonstrates basic usage of the IFNULL function.
...
Query:
```sql
SELECT product_id, product_name, IFNULL(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how IFNULL handles NULL values.
...
Query:
```sql
-- Practical example using IFNULL
...
This example shows a practical application of the IFNULL function.