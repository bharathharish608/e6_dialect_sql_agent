...
Inline IF function (Snowflake compatible)
...
- **Type**: Type of second argument, nullable
- **Description**: Returns the result of the IFF operation.
- **NULL Handling**: Function behavior with NULL inputs depends on the specific operation.

## Usage Notes

- Inline IF function (Snowflake compatible)
- Function accepts BOOLEAN, SAME, SAME
- Return type is Type of second argument, nullable
...
Query:
```sql
SELECT IFF(status = 'active', 1, 0) FROM accounts
```
...
This example demonstrates basic usage of the IFF function.
...
Query:
```sql
SELECT product_id, product_name, IFF(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how IFF handles NULL values.
...
This example shows a practical application of the IFF function.