...
```sql
CASE( <expression1>, <expression2> [ , <expressionN> ... ] )
```

## Arguments

### expression1
- **Type**: Condition/value pairs with optional ELSE
- **Required**: Yes
...
- **Type**: Least restrictive type of result expressions
- **Description**: Returns the result of the CASE operation.
- **NULL Handling**: Function behavior with NULL inputs depends on the specific operation.

## Usage Notes

- Conditional expression that returns different values based on conditions
- Function accepts Condition/value pairs with optional ELSE
- Return type is Least restrictive type of result expressions
...
This example demonstrates basic usage of the CASE function.
...
Query:
```sql
SELECT product_id, product_name, CASE(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how CASE handles NULL values.
...
This example shows a practical application of the CASE function.