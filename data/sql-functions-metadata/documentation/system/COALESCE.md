...
- **Type**: Least restrictive type of arguments
- **Description**: Returns the result of the COALESCE operation.
- **NULL Handling**: Function behavior with NULL inputs depends on the specific operation.

## Usage Notes

- Returns the first non-null value in a list
- Function accepts ANY, ANY, ...
- Return type is Least restrictive type of arguments
...
This example demonstrates basic usage of the COALESCE function.
...
Query:
```sql
SELECT product_id, product_name, COALESCE(price) AS result
FROM products
ORDER BY product_id;
```
...
This example shows how COALESCE handles NULL values.
...
Query:
```sql
-- Practical example using COALESCE
...
This example shows a practical application of the COALESCE function.